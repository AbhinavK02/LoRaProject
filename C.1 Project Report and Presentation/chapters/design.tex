...
The complete functional structure diagram of the system is shown in \ref{fig: fsd}. This illustrates the highest level of the systems design and the different modules which were created to fulfil the requirements. The complete design and revision history can also be found in the project repository on GitHub following this link \url{https://github.com/AbhinavK02/LoRaProject}.
...
\label{sec: 3dDesign}
According to \citeA{joyit_hx711_loadcell}, to reliably detect any weight, the load cell must be allowed to bent as easily as possible and thus be secured within a platform. To prevent any mail from "missing" the detection platform, the platform must cover the entire box area. The design which allows this is shown in Figure \ref{fig: 3dModels}.
...
All of the configuration and functions required for LoRaWAN communication are defined in the \textbf{LoRaConfig.h} file. The libraries required are RadioLib (V7.4.0) by Jan Gromes and Preferences (installed in Arduino IDE by default). The configuration process was also completed with guidance from \citeA{wio_sx1262_xiao_esp32s3_lora_node}.
...
The final website can be run by just running the bash file and afterwards going to this link: \textbf{\href{https://unarithmetically-peppiest-libbie.ngrok-free.dev }{Smart Mailbox Dashboard}}
