...
SensorReading readWeightSensor()
{
  ...
  int weightState = 0;
  long weight = 0;
  long weight_thres_light = 5.0;   // Threshold [g] at which sensor alerts system - light package
  long weight_thres_medium = 50.0; // Threshold [g] at which sensor alerts system - medium package
  long weight_thres_heavy = 100.0; // Threshold [g] at which sensor alerts system - heavy package
  ...
  if (scale.is_ready())
  {
    ...
    Serial.print(weight);
    if (weight > weight_thres_heavy)
    { // If weight is above our threshold
      weightState = 3;
...
SensorReading batteryMonitor()
{
  ...
  // Calculating Voltage at the pin
  float adcVoltage = (adcValue * 3.3) / 4095;
  // Calculating the actual battery voltage using voltage divider ratio
...
uint16_t detectMail(uint8_t *payloadBuffer)
{
  ...
  if (weightState_curr == weightState_prev)
  { // if no change, then someone is peaking 👀
    Serial.println("Tampering Detecting");
    payloadBuffer[len++] = ID_TAMPERING; // Encode Tampering code
  }
  else
  { // If state changed, look at the mail status
